...
/* ═══════════════════════════════════════════
   WAVE CANVAS HOOK
═══════════════════════════════════════════ */
function useWaveGrid(canvasRef: React.RefObject<HTMLCanvasElement | null>, wrapRef: React.RefObject<HTMLDivElement | null>) {
  useEffect(() => {
    ...
    function draw() {
      ...
      const pts: Array<Array<{ px: number; py: number; edgeAlpha: number; waveAmp: number }>> = []
      for (let xi = 0; xi <= COLS; xi++) {
        ...
        for (let yi = 0; yi <= ROWS; yi++) {
          ...
          const waveAmp    = Math.abs(w1 + w3) / MAX_DISP
          pts[xi][yi] = { px, py, edgeAlpha, waveAmp }
        }
      }
...
function MetricCell({ target, suffix, label, sub, delay }: {
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(([e]) => { if (e.isIntersecting) { setVisible(true); el.classList.add('animated') } }, { threshold: 0.3 })
    io.observe(el)
  ...
  return (
    <svg className="logo-mark" viewBox="0 0 38 38" xmlns="http://www.w3.org/2000/svg">
      ...
      <g filter="url(#glow3dG)" transform="translate(1,0)">
        <path d="M 22 9 A 10 10 0 1 0 28 25 L 28 19 L 23 19" fill="none" stroke="url(#gGrad)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
        <path d="M 22 9 A 10 10 0 1 0 28 25 L 28 19 L 23 19" fill="none" stroke="url(#shine)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
      </g>
      <g filter="url(#glow3d)" transform="translate(-1,0)">
        <polyline points="9,9 25,9 9,27 25,27" fill="none" stroke="url(#zGrad)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
        <polyline points="9,9 25,9 9,27 25,27" fill="none" stroke="url(#shine)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
      </g>
    </svg>
  )
}
...
export default function Home() {
  ...
  // ── CSS variable helpers for factor bars (must be camelCase in JSX style) ──
  ...
  return (
    <>
      ...
      <div className={`mob-menu ${mobOpen ? 'open' : ''}`}>
        ...
        {['divisions','ai-tools','digital','automation','contact'].map(id => (
          <a key={id} href={`#${id}`} onClick={() => scrollTo(id)}>
            {id === 'ai-tools' ? 'AI Tools' : id === 'divisions' ? 'Services' : id.charAt(0).toUpperCase() + id.slice(1)}
          </a>
        ))}
      </div>
      ...
      <nav className={scrolled ? 'scrolled' : ''}>
        ...
        <a href="#contact" className="nav-contact" onClick={e => { e.preventDefault(); scrollTo('contact') }}>Start a Project</a>
        <button className="nav-toggle" onClick={() => setMobOpen(true)}><span/><span/><span/></button>
      </nav>
        ...
        <section id="hero">
          <div className="hero-inner">
            ...
            <p className="hero-sub">
              Three divisions. One philosophy: intelligence applied with precision outperforms scale every time. We build for founders who know the difference.
            </p>
            <div className="hero-actions">
              <a href="#divisions" className="btn-primary" onClick={e => { e.preventDefault(); scrollTo('divisions') }}><span>Explore Our Work</span></a>
              <a href="#contact"   className="btn-text"    onClick={e => { e.preventDefault(); scrollTo('contact') }}>Start a Conversation</a>
            </div>
          </div>
      ...
      <section id="metrics">
        <div className="metrics-inner">
          ...
          <MetricCell target={93}    suffix=".2%" label="A+ Grade Win Rate"       sub="9-factor Smart Score algorithm · Validated" delay="0.07s"/>
        ...
        <div className="max-w">
          <div className="divisions-head">
            ...
            <p className="s-body reveal d1">
              <strong>Each division operates independently</strong> — its own toolkit, its own market, its own measure of excellence. Together they form a single thesis: precision intelligence, applied deliberately, changes outcomes.
            </p>
          </div>
          <div className="divisions-grid">
            ...
            <div className="div-card reveal">
              ...
              <p className="div-desc">Proprietary algorithms that rank, score, and surface high-probability trades — built on real backtested data across 96,208 contracts. The only screener that ranks instead of filters.</p>
              <div className="div-items">
                {['OptionsAnalytx — Live Production SaaS','9-Factor Smart Score Algorithm','96,208 Contracts · 488 Symbols · 7 Years','New Financial Verticals In Development'].map(i => (
                  <span key={i} className="div-item">{i}</span>
                ))}
              </div>
              <a href="#ai-tools" className="div-link" onClick={e => { e.preventDefault(); scrollTo('ai-tools') }}>Explore Division →</a>
            </div>
            {/* Div 02 */}
            <div className="div-card reveal d1">
              ...
              <div className="div-icon-wrap">
                <svg viewBox="0 0 24 24" fill="none" strokeWidth="1.2" strokeLinecap="round" stroke="currentColor">
                  ...
                  <polyline points="8,21 16,21"/><line x1="12" y1="18" x2="12" y2="21"/>
                  <line x1="2" y1="13" x2="22" y2="13"/>
                  <rect x="5" y="6" width="5" height="4" rx="0.5"/>
                  <line x1="13" y1="7" x2="18" y2="7"/><line x1="13" y1="9.5" x2="16" y2="9.5"/>
                </svg>
              </div>
              <div className="div-name">Bespoke Digital Agency</div>
              <div className="div-tag">Design · Development · Strategy</div>
              <p className="div-desc">Zero templates. Every pixel, animation, and line of code built to your exact specification — from your color palette through to live deployment. For founders who understand the website is the product.</p>
              <div className="div-items">
                {['Digital Audit & Strategy Report','Interactive Prototype & Design','Next.js Custom Production Build','SEO Foundation & Paid Media Setup'].map(i => (
                  <span key={i} className="div-item">{i}</span>
                ))}
              </div>
              <a href="#digital" className="div-link" onClick={e => { e.preventDefault(); scrollTo('digital') }}>Explore Division →</a>
            </div>
            {/* Div 03 */}
            <div className="div-card reveal d2">
              ...
              <p className="div-desc">AI-powered workflow systems that eliminate manual overhead — from lead response to proposal generation. Built on the Claude API, Zapier, and custom architecture so your business runs while you focus on growth.</p>
              <div className="div-items">
                {['AI Lead Response & Follow-Up','Automated Proposal Generation','Client Intake & Onboarding','Custom AI Solutions'].map(i => (
                  <span key={i} className="div-item">{i}</span>
                ))}
              </div>
              <a href="#automation" className="div-link" onClick={e => { e.preventDefault(); scrollTo('automation') }}>Explore Division →</a>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="oa-layout">
            ...
            <div>
              ...
              <h2 className="s-h2 reveal" style={{ fontSize:'clamp(2.8rem,4.5vw,5rem)' }}>
                <span style={{ color:'var(--gold-pale)' }}>Ω</span>ptions<em>Analytx</em> —<br/>the algorithm<br/>that ranks.
              </h2>
              ...
              <p className="s-body reveal" style={{ marginTop:'0.8rem' }}>
                Every other covered call screener gives you a list. OptionsAnalytx gives you a <strong>ranked verdict</strong>. Every contract across 5,500+ securities is scored 0–100 across 9 proprietary factors simultaneously. The highest-quality trade surfaces to position #1, automatically.
              </p>
              <p className="s-body reveal" style={{ marginTop:'1rem' }}>
                Built on <strong>25 years of covered call trading experience</strong> and validated against 96,208 real contracts across 7 years of market data. Every factor weight is derived from outcome correlation, not intuition.
              </p>
                ...
                <div className="factor-list" id="factorList">
                  {[
                    ...
                    <div key={label as string} className="factor-row">
                      ...
                      <div className="factor-bar-wrap"><div className="factor-fill" style={factorStyle(pct as number)}/></div>
              ...
              <div style={{ marginTop:'1.8rem' }} className="reveal">
                <Link href="https://optionsanalytx.com" target="_blank" className="btn-primary" style={{ display:'inline-flex', alignItems:'center', gap:'0.6rem' }}>
            ...
            <div className="score-card reveal d2">
              ...
              <div className="card-factors" id="cardFactors">
                {[['Prob. Profit',0.82],['OTM Buffer',0.74],['Theta Effic.',0.88],['IV Rank',0.61],['Liquidity',0.91],['Event Risk',0.78]].map(([n,p]) => (
                  <div key={n as string}>
              ...
              <div style={{ padding:'1rem 1.5rem 1.4rem', borderTop:'1px solid rgba(196,150,90,0.07)' }}>
                ...
                {[['2018–2019 Pre-COVID Bull','91.2%','#86C878'],['2020 COVID Crash & Recovery','83.4%','var(--gold)'],['2021 Post-COVID Bull','88.9%','#86C878'],['2022 Bear Market','88.1%','var(--gold)'],['2024 AI-Driven Bull Run','89.7%','#86C878']].map(([label,pct,c]) => (
          ...
          <div className="agency-layout">
            ...
            <div className="reveal" style={{ order:1 }}>
              <div className="phases-wrap">
                {[
                  { num:'01', name:'Digital Audit & Strategy Report',    price:'$500–$750',    desc:'We perform a hands-on expert analysis of your website performance, Google ranking, every local competitor, and your full social media presence — then deliver a prioritized, plain-English roadmap.', why:'Why we\'re different: agencies charge $1,500–$2,500 for a generic scan. We deliver a bespoke roadmap for less than half the cost.' },
                  { num:'02', name:'Design & Interactive Prototype',     price:'$800–$1,200',  desc:'We collaborate on your complete visual identity — full color palette, typography system, feature list, and page-by-page user journey. Then we build a fully interactive browser preview with your real brand, photos, and content in place. Two revision rounds included.', why:'Why it matters: prototyping first eliminates costly mid-build changes and gives you full creative control before costs escalate.' },
                  { num:'03', name:'Full Custom Development',            price:'$2,500–$3,500', desc:'100% custom — no templates, no page builders, no WordPress. We build in Next.js with hand-crafted CSS that makes your site load fast and look pixel-perfect on every device. Booking integrations, contact forms, Facebook Pixel, and Google Analytics 4 all wired in from day one.', why:'Why it matters: custom-built sites rank better, load faster, and convert more than any template — and they look like nothing else in your market.' },
                  { num:'04', name:'SEO Foundation',                    price:'$600–$900',    desc:'92% of Google users never click past page 1 — and the top 3 results capture over 68% of all clicks. We hand-craft custom page titles, meta descriptions, and structured schema data for every page so Google finds, trusts, and ranks your site.', why:'Why we\'re different: most platforms auto-generate weak, generic SEO tags. We write keyword-researched copy for every single page.' },
                  { num:'05', name:'Facebook & Instagram Ads Setup',     price:'$400–$600',    desc:'Full Meta Ads infrastructure: Business Manager, Pixel, custom audiences, 2–3 ad creatives written and designed for your exact customer, full campaign launch, and a plain-English handoff guide. Pixel retargeting delivers up to 3× higher conversion vs. cold ads.', why:'Why it matters: we build audiences around your specific customers by location, age, income, and interests — not broad demographic guesses.' },
                ].map(p => (
            ...
            <div style={{ order:0 }}>
              ...
              <p className="s-body reveal" style={{ marginTop:'1.8rem' }}>We begin every engagement with a complete discovery session — your color palette, typography system, feature requirements, and customer journey. Every interaction, every hover state, every animation is <strong>designed collaboratively and then built in custom Next.js and CSS</strong>. No page builders. No WordPress. No compromise.</p>
              <p className="s-body reveal" style={{ marginTop:'1rem' }}>The result loads fast, ranks well, and looks like nothing else in your market — because it isn&apos;t anything else. It&apos;s yours.</p>
              ...
              <div style={{ marginTop:'2.5rem', padding:'1.4rem', background:'var(--bg-3)', border:'1px solid rgba(196,150,90,0.07)', position:'relative', overflow:'hidden' }} className="reveal">
                ...
                <div style={{ position:'relative', zIndex:1 }}>
                  ...
                  <div style={{ marginTop:'1.2rem', paddingTop:'1rem', borderTop:'1px solid rgba(196,150,90,0.08)', fontFamily:'var(--mono)', fontSize:'0.62rem', color:'var(--text-body)' }}>Monthly retainer available · $200–$350/mo · Updates, reporting, ads management</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <section id="automation" className="s-pad">
        <div className="max-w">
          <div className="auto-head">
            ...
            <p className="s-body reveal d1"><strong>Small business owners lose 40% of their week</strong> to tasks that AI can handle in seconds. We build systems that eliminate the overhead — so every hour you work is an hour only you can fill.</p>
          </div>
          <div className="auto-grid">
            {[
              { num:'01', icon: <path d="M 2 4 h 20 a 1 1 0 0 1 1 1 v 14 a 1 1 0 0 1 -1 1 H 2 a 1 1 0 0 1 -1 -1 V 5 a 1 1 0 0 1 1 -1 Z M 2 8 l 10 6 10 -6"/>, title:'Lead Response Automation', body:'Every inquiry receives an intelligent, personalized response within 60 seconds — 24/7, in your voice. Whether it comes in at 2am on a Sunday or during your busiest day, no lead falls through while you\'re heads-down doing the work.', metric:'↑ 3× more conversions vs. manual follow-up · 40% faster response rate' },
              { num:'02', icon: <><rect x="4" y="2" width="16" height="20" rx="1" stroke="currentColor"/><line x1="8" y1="8" x2="16" y2="8" stroke="currentColor"/><line x1="8" y1="12" x2="16" y2="12" stroke="currentColor"/><line x1="8" y1="16" x2="12" y2="16" stroke="currentColor"/></>, title:'Proposal Generation', body:"Input a client brief — receive a fully scoped, branded, priced proposal in under two minutes. What used to consume an hour of careful writing happens automatically, consistently, every time. More proposals sent means more projects closed.", metric:'↓ 90% reduction in proposal creation time · Consistent professional quality' },
              { num:'03', icon: <><circle cx="12" cy="12" r="10" stroke="currentColor"/><polyline points="12,6 12,12 16,14" stroke="currentColor"/></>, title:'Client Onboarding Systems', body:"The moment a contract is signed, an automated sequence begins — intake form delivered, welcome email sent, project brief collected, kickoff scheduled, access credentials prepared. Your client's experience is professional from the first second.", metric:'Zero manual onboarding steps · Professional first impression, every time' },
              { num:'04', icon: <polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26" stroke="currentColor"/>, title:'Custom AI Solutions', body:"Your business doesn't fit neatly into a template — and neither should your AI. We design and build bespoke automation systems for workflows unique to your operation. Inventory intelligence, content pipelines, pricing optimization — if it can be made smarter with AI, we build it.", metric:'Scoped individually · Built on Claude API, Make, Zapier, and custom code' },
            ].map((card, i) => (
              <div key={card.num} className={`auto-card reveal${i > 0 ? ` d${i}` : ''}`}>
                <div className="auto-num">{card.num}</div>
                <svg className="auto-icon" viewBox="0 0 24 24" fill="none" strokeWidth="1.2" strokeLinecap="round">{card.icon}</svg>
      ...
      <section id="contact">
        <div className="contact-inner reveal">
          ...
          <p className="contact-sub">Whether you&apos;re launching a financial tool, rebuilding your digital presence, or automating your operations — <span>we build for founders who demand precision.</span> No templates. No offshore. No compromise.</p>
          <a href="mailto:info@zaliznyakgroup.com" className="contact-email">info@zaliznyakgroup.com</a>
